...
function MyProfile() {
  ...
  useEffect(() => {
    fetch('https://alumniconnect-fixed-swl8.vercel.app/api/alumni', {
  ...
  const handleSave = async () => {
    ...
    try {
      if (alumniId) {
        const res = await fetch(`https://alumniconnect-fixed-swl8.vercel.app/api/alumni/${alumniId}`, {
          method: 'PUT',
        ...
        if (res.ok) {
          ...
          const newId = (data.alumni && data.alumni[0] && data.alumni[0].id) || (data.data && data.data[0] && data.data[0].id);
          if (newId) setAlumniId(newId);
      ...
      <div style={styles.body}>
        ...
        <div style={styles.formCard}>
          ...
          <button
            ...
            disabled={saving}
          >
            {saving ? 'Saving...' : (alumniId ? 'Update Profile' : 'Save Profile')}
          </button>
        </div>
      </div>
    </div>
