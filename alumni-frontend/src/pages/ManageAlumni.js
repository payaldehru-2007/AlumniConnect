...
const ManageAlumni = () => {
  ...
  const loadAlumni = () => {
    fetch('https://alumniconnect-fixed-swl8.vercel.app/api/alumni', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
  ...
  const handleDelete = async (id) => {
    await fetch(`https://alumniconnect-fixed-swl8.vercel.app/api/alumni/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
    loadAlumni();
  };
