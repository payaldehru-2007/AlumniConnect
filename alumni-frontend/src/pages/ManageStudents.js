...
const ManageStudents = () => {
  ...
  const loadStudents = () => {
    fetch('https://alumniconnect-fixed-swl8.vercel.app/api/auth/students', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json())
  ...
  const handleAdd = async () => {
    ...
    const res = await fetch('https://alumniconnect-fixed-swl8.vercel.app/api/auth/register', {
      method: 'POST',
  ...
  const handleDelete = async (id) => {
    ...
    const res = await fetch(`https://alumniconnect-fixed-swl8.vercel.app/api/auth/delete-student?id=${id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` }
    });
    if (res.ok) loadStudents();
    else setMessage('Error deleting account');
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <table style={styles.table}>
        <thead><tr style={styles.thead}><th style={styles.th}>Full Name</th><th style={styles.th}>Username</th><th style={styles.th}>Actions</th></tr></thead>
        <tbody>
          {students.map((s) => (
...
const styles = {
  ...
  td: { padding: '12px' },
  deleteBtn: { padding: '6px 14px', background: '#e74c3c', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '13px' }
};
